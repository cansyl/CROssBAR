$( function() {
	$('#network').height( screen.height - $('#top_menu').height() - 180 );
    function split( val ) {
	  return val.split(  '|' );
    }
 ...
 $( "#node_search" )
      ...
      .autocomplete({
        ...
        source: function( request, response ) {
			$.ajax( {
			  url: "covid19/largescale_search_in_network.php",
			  dataType: "json",
			  data: {
				term: request.term
			  },
	...
	function apply_high(name){
		$.ajax( {
		  url: "covid19/largescale_take_id_of_node.php",
		  data: {
			name: name
		  },
		  success: function( node_id ) {
			...
			$.ajax({
				type:"POST",
				url:'covid19/largescale_selected_node.php',
	...
	cy.on('select', function(e){
		...
		if(ele.data().source){
			...
			ele.connectedEdges().connectedNodes().addClass('highlightednode');
			ele.connectedEdges().animate({
				style: {width: "3px","opacity":"1","font-size": "15px"}
			});
		}
	});
