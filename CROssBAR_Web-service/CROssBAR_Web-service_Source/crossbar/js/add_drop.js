	...
	if(!$(this).hasClass("list-group-item-secondary")){
		switch($(this).attr('id')){
			...
			case 'kegg_Disease':
				...
				//removed_edges['kegg_dis_path'] = cy.elements('edge[Edge_Type="kegg_dis_path"]');
				//removed_edges['kegg_dis_drug'] = cy.elements('edge[Edge_Type="kegg_dis_drug"]');
			break;
			case 'HPO':
		...
		//var collection = cy.elements('node[Node_Type="'+$(this).attr('id')+'"]');
		cy.add( removed_nodes[$(this).attr('id')] );

		switch($(this).attr('id')){
			case 'Disease':
				cy.add( removed_edges['Disease'] );
				//if($('#HPO').hasClass("btn-primary")){
			...
			case 'kegg_Disease':
				...
				//cy.add( removed_edges['kegg_dis_drug'] );
				//if($('#Drug').hasClass("btn-primary"))
				if(!$('#Drug').hasClass("list-group-item-secondary"))
					cy.add( kegg_dis_drug );
				//cy.add( removed_edges['kegg_dis_path'] );
				//if($('#Pathway').hasClass("btn-primary"))
				if(!$('#Pathway').hasClass("list-group-item-secondary"))
					cy.add( kegg_dis_path );
			break;
			case 'HPO':
				cy.add( removed_edges['HPO'] );
				//if($('#Disease').hasClass("btn-primary")){
